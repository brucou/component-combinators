...
import {
  componentNameInSettings, traceBehaviourSourceFn, traceEventSinkFn, traceEventSourceFn
...
const ANOTHER_RESPONSE = 'another_response';
...
function commandProcessingFnOneResult(command) {
  return $.of({
    request: command,
    response: A_RESPONSE
  })
...
function commandProcessingFnTwoResults(command) {
  return $.from([{
    request: command,
    response: A_RESPONSE,
    index: 0
...
function sinkUpdatingComponent(sources, settings) {
  return {
    ...
    [YET_ANOTHER_DRIVER]: sources[A_CIRCULAR_BEHAVIOR_SOURCE].map(
      x => `circular behaviour emits: ${JSON.stringify(x)}`),
  }
}
...
QUnit.test("main case - InjectCircularSources - behaviour, event (1 result), other", function exec_test(assert) {
  resetGraphCounter();
...
QUnit.skip("main case - InjectCircularSources - behaviour, event (2 results), other", function exec_test(assert) {
  resetGraphCounter();
  const done = assert.async(5);
  const traces = [];
